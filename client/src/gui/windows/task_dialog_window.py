...
class TaskDialogWindow(QDialog):
    ...
    :param readonly: Режим только для чтения (для просмотра задачи).
    """
    ...
    def _apply_readonly(self):
        """Применяет режим только для чтения ко всем полям формы."""
        self._view.line_edit_name.setReadOnly(True)
        self._view.text_edit_description.setReadOnly(True)
        self._view.combobox_executor.setEnabled(False)
        self._view.date_time_plan_start.setReadOnly(True)
        self._view.date_time_plan_deadline.setReadOnly(True)
        self._view.btn_save.setVisible(False)
        self._view.btn_cancel.setText('Закрыть')
    ...
    def get_task_data(self) -> dict:
        ...
        return {
            ...
            'plan_start': plan_start.toString(Qt.ISODate),
            'plan_deadline': plan_deadline.toString(Qt.ISODate)
        }
