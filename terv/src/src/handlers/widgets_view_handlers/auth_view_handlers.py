...
from terv.src.gui.widgets_view.register_view import RegisterView
from terv.src.src.handlers.widgets_view_handlers.base import BaseViewHandler
...
class AuthViewHandler(BaseViewHandler):
    ...
    def __init__(self, auth_view: AuthView, data_const: DataStructConst()):
    ...
    def try_to_auth(self):
        ...
        if not login:
            self._auth_view.set_error_login()
        if not password:
            self._auth_view.set_error_password()
...
class RegisterViewHandler(BaseViewHandler):
    ...
    def __init__(self, register_view: RegisterView, data_const: DataStructConst = DataStructConst()):
    ...
    def try_to_register(self):
        ...
        if not login:
            self._register_view.set_error_login()
        if not password:
            self._register_view.set_error_password()
        if not email:
            self._register_view.set_error_email()
